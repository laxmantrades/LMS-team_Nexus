...
// ------------------------------
  ...
  {book.bookImage && (
    <img
      src={book.bookImage}
      alt={book.title}
      className="w-full h-96 object-contain rounded-md mb-4 bg-slate-900"
    />
  )}

  <h1 className="text-2xl font-bold mb-2 text-indigo-100">{book.title}</h1>
  <p className="text-slate-300 mb-1">by {book.author}</p>
  {book.genre && (
    <p className="text-slate-400 mb-2">Genre: {book.genre}</p>
  )}
  <p className="text-slate-200 mb-4">{book.description}</p>
  ...
  {banner && (
    <div
      className={`p-3 my-4 rounded border border-slate-700 bg-slate-900/70 ${banner.className}`}
    >
      ...
      <div className="mt-2 text-sm text-slate-200">
        ...
        <div>
          ...
          {String(reservation?.approved ?? false)}
        </div>
  ...
  {!isBorrowed && !isReserved && (
    <div className="mt-6">
      ...
      <textarea
        ...
        className="w-full border border-slate-700 bg-slate-900 text-slate-100 placeholder:text-slate-500 rounded p-2 mb-3 min-h-[100px] resize-vertical"
